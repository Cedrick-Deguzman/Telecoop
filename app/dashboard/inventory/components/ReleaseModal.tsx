...
export default function ReleaseModal({ item, onClose, onSuccess }: Props) {
  const isSerialized = item.category.type === 'serialized';
  const inStockSerials = item.serials?.filter(s => s.status === 'in_stock') ?? [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/inventory/${item.id}/release`, {
        method: 'POST',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Release Stock</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-6">
          {isSerialized ? (
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-700">
                Select Units to Release ({inStockSerials.length} in stock)
              </label>
              {inStockSerials.length === 0 ? (
                <p className="text-sm text-slate-500">No units in stock.</p>
              ) : (
            ...
            <div>
              <label className="text-sm font-medium text-slate-700">
                Quantity ({item.unit}) — Available: {item.quantity}
              </label>
              <input
                type="number"
                min="0.01"
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              placeholder="Purpose, destination, etc."
              value={notes}
